...
**Timeout**: → Integer representing execution time limit(when timeout limit is reached, the action will throw an error)
**Single result**: → Output that will be set for single value outputs.
**List result**: → Output that will be set for list value outputs.
**Error**: → String representing errors resulted from the execution of the script (the output is set only when an error occurs, otherwise it’s **null**)
